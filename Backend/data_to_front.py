...
def get_all_data_from_database(response_from_front):
    ...
    # for index in range(1):
        ...
        if not flag:
            result_list.append(list_of_dicts[index])
            if list_of_dicts[index]['area']["name"] not in region_list:
                region_list.append(list_of_dicts[index]['area']["name"])
    # if  (result_list != []):
    #     print(result_list)
    print(region_list)
    sorted_list = sorted(result_list, key=lambda x: x['area']['id'])
    return {'resumes' :sorted_list, 'region_list': region_list}
